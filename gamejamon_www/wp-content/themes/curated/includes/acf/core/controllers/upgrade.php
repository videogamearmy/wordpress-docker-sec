...
class acf_upgrade
{
	...
	*/
	...
	function admin_menu()
	{
		...
		// update admin page
		// disable___add_submenu_page('edit.php?post_type=acf', __('Upgrade','acf'), __('Upgrade','acf'), 'manage_options','acf-upgrade', array($this,'html') );
	}
		...
		switch($_POST['version'])
		{
			...
			*--------------------*/
			...
			case '3.1.8':
				...
				// vars
				$message = __("Migrating options values from the","acf").' '.$wp_postmeta.' '.__('table to the','acf').' '.$wp_options.' '.__('table','acf') . '...';
